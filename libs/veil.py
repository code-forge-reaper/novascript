    ...
    def finalise_rule():
        ...
        if current_pattern is not None:
            try:
                ...
                raise RulesError(f"Invalid regex pattern: {current_pattern} – {exc}")
...
# ---------------------------------------------------------------------------
# Execution (API) – exactly as original: exec with optional sandbox
# Output goes to stdout/stderr, no capture.
# ---------------------------------------------------------------------------
...
def veil(path, env = None, rules="lang.reg"):
    with open(path) as f:
        input_text = f.read()
    ...
    execute_code(formatted, env)        # runs, prints directly
